...
test.beforeAll(async ({ request }) => {
  let response = await request.post('/goals-v2-api.php?action=track', { data: { id: trackId, title: 'AUDIT Permis de conduire', category: 'AUDIT Projet personnel' } });
...
for (const viewport of [
  ...
  test(`nouveau flux Objectifs ${viewport.width}x${viewport.height}`, async ({ page }) => {
    ...
    await expect(goal.getByText('7 jours restants')).toBeVisible();
...
test('menus secondaires et formulaire daté', async ({ page }) => {
  ...
  await expect(page.getByRole('dialog', { name: 'Modifier l’objectif' })).toBeVisible();
  await page.getByRole('dialog', { name: 'Modifier l’objectif' }).getByRole('button', { name: 'Fermer' }).click();
  await goal.getByRole('button', { name: 'Ajouter un sous-objectif' }).click();
  const dialog = page.getByRole('dialog', { name: 'Nouveau sous-objectif' });
